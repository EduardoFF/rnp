...
void MyCplexSolver::addRelayVar(string rid)
{
...
void MyCplexSolver::addFXVar(string id)
{
  int retCPX;
  int ccnt;
  char ctype;
...
int MyCplexSolver::getFXVarIndex(string i)
{
...
int MyCplexSolver::getYVarIndex(string i)
{
  ITERATOR(m_varY) it = m_varY.find(i);
  assert(it != m_varY.end());
  return it->second;
}
int MyCplexSolver::getXVarIndex(string i, string j)
{
...
int MyCplexSolver::getBXVarIndex(string i, string j)
{
 ...
 * */
void MyCplexSolver::addFlowCons()
{
  int ccnt;
  int rcnt;
  int nzcnt;
...
void MyCplexSolver::addFlowEdgeCons()
{
...
void MyCplexSolver::addBaseCons()
{
  /*
...
void MyCplexSolver::addFlowSelCons()
{
  /*
...
void MyCplexSolver::setObjFunc()
{
  ...
  int *indeces = (int *)malloc((m_varX.size()+m_varFX.size()+m_varY.size())*sizeof(int));
  double *values = (double *)malloc((m_varX.size()+m_varFX.size()+m_varY.size())*sizeof(double));
  ...
  //	printf("flow_neighbor_penalty %f\n",flow_neighbor_penalty);
...
void MyCplexSolver::addBWCapacityCons()
{
  /*
...
void MyCplexSolver::setRelayBnds(int mink, int maxk)
{
  /*
...
void MyCplexSolver::setFlowRelayCons()
{
  addRelaySelCons();
}
...
bool MyCplexSolver::solve()
{
  ...
  double cpx_trelim = prob.mycplex_params.trelim; // MEMORY LIMIT
  double cpx_workmem = prob.mycplex_params.workmem; // MEMORY LIMIT
  int cpx_nodefileind = prob.mycplex_params.nodefileind; // MEMORY LIMIT
  int cpx_mipemphasis = prob.mycplex_params.mipemphasis;
  int cpx_threads = prob.mycplex_params.threads; // NUMBER OF THREADS (0 means automatic=ALL)
  ...
  int cpx_writemps = 1;
  int cpx_writeparam = 1;
  //int cpx_writemps = prob.mycplex_params.writemps;
  /** execution flags **/
...
#define DEBUGMIPSTART 0
  ...
  if( prob.mycplex_params.logToFile)
  {
    ...
    if(retCPX )
      fprintf(stderr, "Cannot close cpxout file\n");
  }
